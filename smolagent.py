...
class ReactTSProjectManager:
    ...
    def create_component(self, name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            (component_dir / f"{name}.tsx").write_text(
...
""",
            ...
            (component_dir / f"{name}.module.css").write_text(
                f".{name.lower()}-container {{\n  /* Estilos aquí */\n}}",
                encoding="utf-8",
            )

            return (
                True,
                f"✅ Componente {name} creado en {component_dir.relative_to(self.project_root)}",
            )
...
def setup_project_environment(manager: ReactTSProjectManager):
    ...
    if not (manager.project_root / "package.json").exists():
        print(
            "⚠ No se encontró package.json. Algunas funciones pueden no trabajar correctamente."
        )
    ...
    while True:
        ...
        if choice == "1":
            ...
            component_name = input(
                "\nIngrese el nombre del componente (ej. Button): "
            ).strip()
            if component_name:
